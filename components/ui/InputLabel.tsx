export default function InputLabel({ forInput, value, className }:{forInput:string,value: string,className?:string}) {
    return (
        <label 
        htmlFor={forInput} 
        className={`absolute text-sm text-semiDark dark:text-d_semiLight duration-300 transform -translate-y-3 scale-75 top-1 z-10 origin-[0] bg-light dark:bg-d_semiDark px-2 peer-focus:px-2 
